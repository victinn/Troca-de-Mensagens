...
public class modelRecebidas extends AbstractTableModel{
    ArrayList<Mensagens> mensagens = new ArrayList();
    String[] colunas = {"Remetente", "Corpo"};
    
    public void SelecionarRecebidas(int id_destinatario) throws SQLException{
        MensagensDAO dao = new MensagensDAO();
        mensagens = dao.SelecionarRecebidas(id_destinatario);
    ...
    public Object getValueAt(int linha, int coluna) {
       switch(coluna){
           ...
           {
               try {
                   return pegarNome(mensagens.get(linha).getId());
               } catch (SQLException ex) {
                   Logger.getLogger(modelRecebidas.class.getName()).log(Level.SEVERE, null, ex);
               }
           }
           case 1:
